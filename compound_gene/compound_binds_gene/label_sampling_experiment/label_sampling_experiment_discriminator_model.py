...
dev_baseline_df = pd.read_table("results/CbG/results/dev/baseline_marginals.tsv_dev.tsv")
...
dev_baseline_df = dev_baseline_df.merge(candidate_dfs['dev'][["curated_cbg", "candidate_id"]])
fpr, tpr, _ = roc_curve(dev_baseline_df.curated_cbg, dev_baseline_df.pos_class_marginals)
precision, recall, _ = precision_recall_curve(dev_baseline_df.curated_cbg, dev_baseline_df.pos_class_marginals)
...
base_line = pd.read_table("results/CbG/results/dev/baseline_marginals.tsv_dev.tsv")
base_line = base_line.merge(candidate_dfs['dev'][["candidate_id", "curated_cbg"]])
neg_class = sum(base_line.query("curated_cbg==0").pos_class_marginals.apply(lambda x: 1 if x > 0.5 else 0).values == base_line.query("curated_cbg==0").curated_cbg.values)
pos_class = sum(base_line.query("curated_cbg==1").pos_class_marginals.apply(lambda x: 1 if x > 0.5 else 0).values == base_line.query("curated_cbg==1").curated_cbg.values)

dev_baseline = pd.DataFrame([
    [neg_class/base_line.query("curated_cbg==0").shape[0], 0, 0],
    [pos_class/base_line.query("curated_cbg==1").shape[0], 0, 1],
...
test_baseline_df = pd.read_table("results/CbG/results/test/baseline_marginals.tsv_test.tsv")
...
test_baseline_df = test_baseline_df.merge(candidate_dfs['test'][["curated_cbg", "candidate_id"]])
fpr, tpr, _ = roc_curve(test_baseline_df.curated_cbg, test_baseline_df.pos_class_marginals)
precision, recall, _ = precision_recall_curve(test_baseline_df.curated_cbg, test_baseline_df.pos_class_marginals)
...
base_line = pd.read_table("results/CbG/results/test/baseline_marginals.tsv_test.tsv")
base_line = base_line.merge(candidate_dfs['test'][["candidate_id", "curated_cbg"]])
neg_class = sum(base_line.query("curated_cbg==0").pos_class_marginals.apply(lambda x: 1 if x > 0.5 else 0).values == base_line.query("curated_cbg==0").curated_cbg.values)
pos_class = sum(base_line.query("curated_cbg==1").pos_class_marginals.apply(lambda x: 1 if x > 0.5 else 0).values == base_line.query("curated_cbg==1").curated_cbg.values)
...
def get_disc_performance(
    ...
    data_set_category="dev", curated_field="curated_cbg",
):
...
test_performance_dict, test_class_performance_dict = get_disc_performance(
    ...
    data_set_category="test", curated_field="curated_cbg"
)
