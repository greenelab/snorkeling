...
biomarker_indicators = {
    ...
    "serum", "expressed on", "overexpression of", "plasma", "over-expression"
    }
...
upregulates = {
    ...
    "was enhanced", "over-expression", "overexpression"
}
downregulates = {
...
method_indication = {
    ...
    "compared to", "were analyzed", "evaluated in", "examination of", "examined in",
    "quantified in" "quantification by", "we review", "(were|was) measured", "we(re)? studied", 
    "we measured", "derived from", "(are|is) discussed", "to measure", "to study",
    ...
    "we examine(d)?", "(was|we|were) evaluated", "to establish", "were selected", "authors determined",
...
def LF_DG_IS_BIOMARKER(c):
    ...
    if LF_DG_METHOD_DESC(c) or LF_DG_TITLE(c):
        return 0
    elif re.search(ltp(biomarker_indicators) + r".*{{B}}", get_tagged_text(c), flags=re.I):
        return 1
...
def LF_DaG_ASSOCIATION(c):
    ...
    if LF_DG_METHOD_DESC(c) or LF_DG_TITLE(c):
        return 0
    elif re.search(r'(?<!not )(?<!no )' + ltp(direct_association), get_text_between(c), flags=re.I):
        return 1
