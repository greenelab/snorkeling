...
dev_labels = pd.read_csv("input/gig_dev_labels.tsv", sep="\t")
dev_labels.head()
...
gen_model_results_dev_df = pd.read_csv(
    "../label_sampling_experiment/results/GiG/results/dev_sampled_results.tsv", 
    sep="\t"
)
...
gen_predicton = (
    pd.read_csv(
        "../label_sampling_experiment/results/GiG/marginals/tune/28_sampled_dev.tsv",
        sep="\t"
    )
...
precision_recall_fscore_support(
    ...
    .curated_gig
    .values, 
    disc_model_dict[28]['0'].apply(lambda x: 1 if x > 0.5 else 0),
    average='binary'
)
...
test_labels = pd.read_csv("input/gig_test_labels.tsv", sep="\t")
test_labels.head()
...
gen_model_results_test_df = pd.read_csv(
    "../label_sampling_experiment/results/GiG/results/test_sampled_results.tsv", 
    sep="\t"
)
...
gen_predicton = (
    pd.read_csv(
        "../label_sampling_experiment/results/GiG/marginals/test/28_sampled_test.tsv",
        sep="\t"
    )
    .assign(candidate_id=candidate_df.candidate_id.values.tolist())
    .query(f"candidate_id in {disc_model_dict[value].candidate_id.values.tolist()}")
