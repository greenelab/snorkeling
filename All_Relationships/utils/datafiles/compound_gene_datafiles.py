...
pair_df.to_csv("data/compound_gene/compound_binds_gene/compound_gene_pairs_binds_full_map.csv", index=False, float_format='%.5g')
...
for r in tqdm_notebook(pd.read_csv("data/compound_gene/compound_binds_gene/compound_gene_pairs_binds_full_map.csv", chunksize=1e6, dtype={'entrez_gene_id': int})):
    ...
    merged_df.to_csv("data/compound_gene/compound_binds_gene/compound_gene-pairs_binds_mapping.csv", mode='a', index=False)


# Memory issues occur when I try to build the full dataframe
# Have to rely on command line to remedy this issue
os.system(
    "head -n 1 data/compound_gene/compound_binds_gene/compound_gene-pairs_binds_mapping.csv > data/compound_gene/compound_binds_gene/compound_gene_pairs_binds.csv;" +
    "cat data/compound_gene/compound_binds_gene/compound_gene-pairs_binds_mapping.csv |  awk -F ',' '{if($8==1) print $0}' >> data/compound_gene/compound_binds_gene/compound_gene_pairs_binds.csv"
)

cbg_map_df = pd.read_csv("data/compound_gene/compound_binds_gene/compound_gene_pairs_binds.csv")